...
# Scrape the tables.
extracted_table <- table_scrape()

# Exercise b
scrapeAdditionalData <- function(university_data) {
  # Initialize empty lists to store additional data
  coordinates <- vector("character", length = nrow(university_data))
  clean_endowment <- vector("character", length = nrow(university_data))
  clean_undergrad <- vector("numeric", length = nrow(university_data))
  clean_postgrad <- vector("numeric", length = nrow(university_data))
  
  endowment <- vector("character", length = nrow(university_data))
  undergrad <- vector("numeric", length = nrow(university_data))
  postgrad <- vector("numeric", length = nrow(university_data))
  total_students <- vector("numeric", length = nrow(university_data))
  
  #university_data <- extracted_table[[1]]
  ...
  for (i in 1:nrow(university_data)) {
    ...
    endowment[i] <- university_page %>%
      html_nodes("a[title=\"Financial endowment\"]") %>%
      html_nodes(xpath = "./following::td[1]") %>%
      html_text()
    
    clean_endowment[i] <- gsub("\\[\\d+\\]", "", endowment[i])
    
    # Extracting total number of students
    #undergrad[i] <- university_page %>%
 #     html_nodes(".infobox tbody tr:contains('Undergraduates') td:nth-child(2)")%>%
  #    html_text() %>%
   #   as.numeric()
    #clean_undergrad[i] <- as.numeric(gsub("\\[\\d+\\]", "", gsub(",", "", undergrad)))
    
#    postgrad[i] <- university_page %>%
#      html_nodes(".infobox tbody tr:contains('Postgraduates') td:nth-child(2)") %>%
 #     html_text() %>%
#      as.numeric()
 #   clean_postgrad[i] <- as.numeric(gsub("\\[\\d+\\]", "", gsub(",", "", postgrad)))
#    
    #total_students[i] <- sum(clean_undergrad[[i]], clean_postgrad[[i]])
    #total_students[i] <- sum(undergrad[i], postgrad[i])
  }
